...
public static class ImageToTensor
{
    ...
    public static DenseTensor<sbyte> ToCHWInt8(string imagePath, int targetH, int targetW)
    {
        using var bmp = SKBitmap.Decode(imagePath);
        if (bmp == null) throw new FileNotFoundException(imagePath);
        float scale = Math.Min((float)targetW / bmp.Width, (float)targetH / bmp.Height);
        ...
        var tensor = new DenseTensor<sbyte>(new[] { 1, 3, targetH, targetW });
        for (int y = 0; y < targetH; y++)
        {
            for (int x = 0; x < targetW; x++)
            {
                var color = canvasBmp.GetPixel(x, y);
                tensor[0, 0, y, x] = (sbyte)(color.Red - 128);
                tensor[0, 1, y, x] = (sbyte)(color.Green - 128);
                tensor[0, 2, y, x] = (sbyte)(color.Blue - 128);
            }
        }
        return tensor;
    }
