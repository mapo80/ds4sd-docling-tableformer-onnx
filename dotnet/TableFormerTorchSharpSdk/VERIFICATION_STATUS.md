...
- **Implementazione**: lo script `scripts/benchmark_docling_python.py` esegue Docling sul sottoinsieme FinTabNet benchmark, salva il report in `results/tableformer_docling_fintabnet_python.json` e, salvo uso esplicito di `--skip-reference-check`, interrompe l'esecuzione se il JSON diverge dal riferimento. Il tool `.NET` `TableFormerTorchSharpSdk.Benchmarks` (console app target `net8.0`) replica la pipeline TorchSharp end-to-end, misura i tempi tramite `Stopwatch`, serializza `predictions`/`timings_ms` in `results/tableformer_docling_fintabnet_dotnet.json` e verifica in forma canonica la parità con il dump Python, a meno che non venga specificato `--skip-reference-check`.
- **Come verificare**:
  ...
  2. Esegui il benchmark TorchSharp: `dotnet run --project dotnet/TableFormerTorchSharpSdk.Benchmarks -- --output results/tableformer_docling_fintabnet_dotnet.json`. Il tool scarica (se necessario) gli artifact in `dotnet/artifacts_benchmark_cache`, salva i tempi per pagina e termina con codice diverso da zero se il JSON non è identico al riferimento (a meno di `--skip-reference-check`).
...
- **Risultato corrente**: ⚠️ La run Python del 2025-10-19 (ripetuta il 2025-10-19 con `--skip-reference-check`) produce JSON differenti rispetto a `results/tableformer_docling_fintabnet.json` e tempi pari a 4164,38 ms e 2564,04 ms. Il benchmark TorchSharp, anch'esso eseguito con `--skip-reference-check`, registra 5635,75 ms e 4612,46 ms ma serializza un output diverso dal dump Python. La verifica resta **BLOCCATA** finché la pipeline Python non verrà riallineata e i due report potranno coincidere senza disattivare il controllo di riferimento.
